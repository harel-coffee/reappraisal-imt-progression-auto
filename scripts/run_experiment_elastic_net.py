...
def find_alpha_range(X, y, n_alphas=1000):
    ...
    alphas = _alpha_grid(X = X_transform, y = y, l1_ratio = 0.5, n_alphas = n_alphas)
...
def run(X, y, cv_outer = LeaveOneOut(), n_alphas = 1000):
    ...
    for train_index, test_index in tqdm(cv_outer.split(X)):
        ...
        pip = make_pipeline(VarianceThreshold(),
                            PCA(),
                            ElasticNet(max_iter=1e6, l1_ratio = 0.5),
                            memory=memory)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Run a particular experiment using an ElasticNet estimator')
