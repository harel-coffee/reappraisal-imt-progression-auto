...
# -*- coding: utf-8 -*-
"""
Script to run the main predictive analyses discarding the 10
subjects that did not reappraise.
...
def load_input(data_dir, cont_type):
    try:
        input_data = np.load(opj(data_dir, "input_data_sensitivity.npz"))
    except:
...
def load_target(data_dir, target_var):
    try:
        df = pd.read_csv(opj(data_dir, "target_data_sensitivity.csv"))
    except:
